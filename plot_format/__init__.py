...
class plot_format(rtl,eldo,thesdk):
    @property
    def _classfile(self):
    ...
    def run(self,*arg):
        ...
        if self.model=='py':
          ...
          if self.model=='vhdl':
              ...
              # Example of defining supplies (not used here because the example plot_format has no supplies)
...
if __name__=="__main__":
    ...
    from  plot_format.controller import controller as plot_format_controller
    import pdb
    length=1024
    rs=100e6
    indata=np.random.randint(2,size=length).reshape(-1,1);
    #indata=np.random.randint(2,size=length)
    controller=plot_format_controller()
    controller.Rs=rs
    #controller.reset()
